...
void onChessPieceSelect(int pX, int pY) {
  ...
  if (selectedBoardPosition > -1) {
    ...
    BoardSquare *originalBoardSquare = &cels[selectedBoardPosition];
    if (selectedBoardPosition != currentSelectedBoardPosition && boardSquare->forceColor) {
      resetBoardColors();

      originalBoardSquare->hasPiece = false;

      if (boardSquare->hasPiece && boardSquare->piece.type == King) {
        boardSquare->piece = originalBoardSquare->piece;
        glutPostRedisplay();

        isGameInteractive = false;
        ModalWindow::show(boardSquare->piece.isWhite);
      }

      boardSquare->piece = originalBoardSquare->piece;
      boardSquare->piece.hasMoved = true;
  ...
  for (int i = 0; i < 8; i++) {
    for (int j = 0; j < 8; j++) {
      ...
      if (boardSquare.hasPiece) {
        ...
        glVertex2i(originX + i * boardSquareSize + 12, originY + j * boardSquareSize + 12);

        glTexCoord2f(1.0, 0.0);
        glVertex2i(originX + (i + 1) * boardSquareSize - 12, originY + j * boardSquareSize + 12);

        glTexCoord2f(1.0, 1.0);
        glVertex2i(originX + (i + 1) * boardSquareSize - 12, originY + (j + 1) * boardSquareSize - 12);

        glTexCoord2f(0.0, 1.0);
        glVertex2i(originX + i * boardSquareSize + 12, originY + (j + 1) * boardSquareSize - 12);
